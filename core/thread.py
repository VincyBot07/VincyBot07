...
class Thread:
    ...
    def __init__(
        ...
        if isinstance(recipient, int):
            ...
            if recipient.bot:
                raise CommandError("Il mittente non può essere un bot.")
    ...
    def __repr__(self):
        return f'Stanza(recipient="{self.recipient or self.id}", channel={self.channel.id})'
    ...
    async def setup(self, *, creator=None, category=None):
        ...
        try:
            channel = await self.bot.modmail_guild.create_text_channel(
                ...
                reason="Sto creando il canale per la stanza.",
            )
        except discord.HTTPException as e:  # Failed to create due to missing perms.
            logger.critical("Non è stato possibile creare la stanza.", exc_info=True)
            self.manager.cache.pop(self.id)

            embed = discord.Embed(color=self.bot.error_color)
            embed.title = "Non è stato possibile creare la stanza."
            embed.description = str(e)
            embed.add_field(name="Mittente", value=recipient.mention)
        ...
        try:
            ...
            logger.error("Non è stato possibile mettere il log nel database.", exc_info=True)
            log_url = log_count = None
            # ensure core functionality still works

        await channel.edit(topic=f"ID utente: {recipient.id}")
        self.ready = True
        ...
        async def send_genesis_message():
            ...
            try:
                ...
                logger.error("Fallito inaspettatamente:", exc_info=True)
    ...
    def _format_info_embed(self, user, log_url, log_count, color):
        ...
        if member is not None:
            ...
            embed.description += f", è entrato {days(joined)}"

            if member.nick:
                embed.add_field(name="Nickname", value=member.nick, inline=True)
            if role_names:
                embed.add_field(name="Ruoli", value=role_names, inline=True)
            embed.set_footer(text=footer)
        else:
            embed.set_footer(text=f"{footer} • (non nel server principale)")
        ...
        if member is None or len(mutual_guilds) > 1:
            embed.add_field(
                name="Server in comune", value=", ".join(g.name for g in mutual_guilds)
            )
    ...
    async def _close(
        ...
        try:
            ...
            logger.error("Stanza già chiusa: %s.", e)
            return
        ...
        if isinstance(log_data, dict):
            ...
            desc = "Non è stato possibile risolvere l'URL del logviewer."
            log_url = None
        ...
        event = "Stanza chiusa come programmato" if scheduled else "Stanza chiusa"
    ...
    async def find_linked_messages(
        ...
        if message1 is not None:
            if (
                ...
                raise ValueError("Messaggio della stanza non valido.")

        elif message_id is not None:
            try:
                message1 = await self.channel.fetch_message(message_id)
            except discord.NotFound:
                raise ValueError("Messaggio della stanza non trovato.")
        ...
        async for msg in self.recipient.history():
            ...
            try:
                ...
                continue
        raise ValueError("Messaggio privato non trovato.")
    ...
    async def find_linked_message_from_dm(self, message, either_direction=False):
        ...
        async for linked_message in self.channel.history():
            ...
            if int(msg_id) == message.id:
                return linked_message
        raise ValueError("Il messaggio del canale della stanza non è stato trovato.")
            ...
            if self.close_task is not None:
                ...
                tasks.append(
                    self.channel.send(
                        embed=discord.Embed(
                            color=self.bot.error_color,
                            description="La chiusura programmata è stata annullata.",
                        )
                    )
                )
    ...
    async def send(
        ...
        for url, filename in images:
            if not prioritize_uploads or (is_image_url(url) and not embedded_image and filename):
                ...
                if filename:
                    embed.add_field(name="Immagine", value=f"[{filename}]({url})")
                embedded_image = True
                ...
                img_embed.url = url
                img_embed.set_footer(text=f"Caricamento immagine aggiuntiva ({additional_count})")
        ...
        for url, filename in attachments:
            embed.add_field(
                name=f"File caricato ({file_upload_count})", value=f"[{filename}]({url})"
            )
            file_upload_count += 1
        ...
        if from_mod:
            ...
            if anonymous and isinstance(destination, discord.TextChannel):
                embed.set_footer(text="Risposta anonima")
            # Normal messages
            elif not anonymous:
            ...
            embed.set_footer(text=f"ID messaggio: {message.id}")
        ...
        try:
            ...
            logger.warning("Canale non trovato.")
            raise
        ...
        if recipient is None and channel is not None:
            ...
            if thread is None:
                ...
                if thread is not None:
                    ...
                    await channel.edit(topic=f"ID utente: {user_id}")
            return thread
